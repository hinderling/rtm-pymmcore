...
| `test_examples.py` | Executes `examples/live_experiment.ipynb` and both `templates/` notebooks on the virtual microscope (run verbatim with small frame counts; `gui` cells dropped; the re-analysis paths cell swapped for a generated source run); checks README anchors the notebooks link to. Marker `examples`; needs the `virtual-microscope` extra. |
