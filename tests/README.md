...
| `test_examples.py` | Executes `examples/*.ipynb` and both templates on the virtual microscope (TODO cells swapped for fillers, `gui` cells dropped); checks README anchors the notebooks link to. Marker `examples`; needs the `virtual-microscope` extra. |
